...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # model statistics
    flops, params = cal_params_flops(base_model)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        print_log('[BEST MODEL] acc = %.6f' %
                  (best_metrics.acc), logger=logger)
        GB = 1024. * 1024. * 1024.
        gpu_memory = torch.cuda.max_memory_allocated()/GB
        res_gpu_memory = torch.cuda.max_memory_reserved()/GB
        print_log('[GPU Mem] MEM = %.3f GB | Reserved MEM = %.3f GB' %
                  (gpu_memory, res_gpu_memory), logger=logger)
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger=None):
    ...
    with torch.no_grad():
        ...
        res_gpu_memory = torch.cuda.max_memory_reserved() / GB
        print_log('[Val GPU Mem] MEM = %.3f GB | Reserved MEM = %.3f GB' %
                  (val_gpu_memory, res_gpu_memory), logger=logger)
...
def validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger=None, times=10):
    ...
    with torch.no_grad():
        ...
        GB = 1024. * 1024. * 1024.
        val_gpu_memory = torch.cuda.max_memory_allocated() / GB
        print_log('[Val GPU Mem] MEM = %.3f GB' %
                  val_gpu_memory, logger=logger)
    ...
    with torch.no_grad():
        ...
        for time in range(1, 300):  # 300
            this_acc = test_vote(base_model, test_dataloader,
                                 1, None, args, config, logger=logger, times=10)
            if acc < this_acc:
                acc = this_acc
...
def test_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger=None, times=10):
    ...
    with torch.no_grad():
        ...
        print_log('[Vote GPU Mem] MEM = %.3f GB' %
                  gpu_memory_vote, logger=logger)
