...
def parse_args():
    ...
    parser.add_argument('--root', type=str, default='/dataset/jinlei/ShapeNetPart/shapenetcore_partanno_segmentation_benchmark_v0_normal/',
                        help='data root')
...
def main(args):
    ...
    def cal_params_flops(model):
        ...
        def input_constructor(input_shape):
            return {
                'pts': torch.randn(*input_shape[0]).cuda(),  # 模拟输入 (3, 2048)
                'cls_label': torch.randn(*input_shape[1]).cuda()  # 模拟输入 (1, 16)
            }
        # model.eval()
        input_shapes = ((1, 3, 2048), (1, 1, 16))
        # input_label = tuple([1, 16])
        flops, params = get_model_complexity_info(model, input_shapes, input_constructor=input_constructor)
        log_string(f'FLOPs: {flops}')
        log_string(f'PARAMs: {params}')

    cal_params_flops(classifier)
