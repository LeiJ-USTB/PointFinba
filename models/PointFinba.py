...
from .rwkv6 import RWKV_Tmix_x060, RWKV_CMix_x060
from .rope import *
...
class GLG(nn.Module):
    def __init__(self, dim, k_group_size, act_layer=nn.SiLU, norm_layer=nn.LayerNorm, cls=True):
        super().__init__()
        self.cls = cls
    ...
    def forward(self, feat, idx, dist):
        ...
        if self.cls:
            knn_x = torch.cat((cls_token, knn_x), dim=1)  # B G+1 C : 1 129 384
        return knn_x
...
class PointFinbaBlock(nn.Module):
    def __init__(self,
        ...
        if layer_id % 2 == 1:
            ...
            self.mixer = RWKV_Tmix_x060(dim, num_heads, n_layer, layer_id)
            self.mixer2 = RWKV_CMix_x060(dim, n_layer, layer_id)
...
class PointFinbaEncoder(nn.Module):
    def __init__(self, k_group_size=8, embed_dim=768, depth=4, drop_path_rate=0., num_heads=6, cls=True):
        super().__init__()
        self.k_group_size = k_group_size
...
class PointFinba(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        expand = 2 if self.cls else 1
        self.cls_head_finetune = nn.Sequential(
            nn.Linear(self.trans_dim * expand, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(256, 256),
    ...
    def forward(self, pts):
        ...
        if self.cls:
            concat_f = torch.cat(
                [x[:, 0], x[:, 1:].max(1)[0] + x[:, 1:].mean(1)[0]], dim=-1)
        else:
...
# Pretrain model
class MaskPointFinba(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
        ...
        self.k_group_size = config.center_local_k
        self.num_group = config.num_group
        print_log(f'[args] {config}', logger='Transformer')
        # embedding
        self.encoder_dims = config.encoder_dims
        self.encoder = Encoder(encoder_channel=self.encoder_dims)
...
@MODELS.register_module()
class Point_MAE_Finba(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[Point_MAE] ', logger='Point_MAE')
        self.config = config
        self.trans_dim = config.trans_dim
        self.MAE_encoder = MaskPointFinba(config)
        self.group_size = config.group_size
        self.num_group = config.num_group
        self.drop_path_rate = config.drop_path_rate
        self.cls = config.cls
        ...
        dpr = [x.item() for x in torch.linspace(
            0, self.drop_path_rate, self.decoder_depth)]
        ...
        print_log(
            f'[Point_MAE] divide point cloud into G{self.num_group} x S{self.group_size} points ...', logger='Point_MAE')
        self.group_divider = Group(
            num_group=self.num_group, group_size=self.group_size)
    ...
    def forward(self, pts, vis=False, **kwargs):
        ...
        if self.cls:
            # x_vis = x_vis[:, 1:]
            mask_pos = self.mask_pos.expand(B, -1, -1)
            pos_emd_vis = torch.cat([mask_pos, pos_emd_vis], dim=1)
        ...
        B, M, C = x_rec.shape
        rebuild_points = self.increase_dim(x_rec.transpose(1, 2)).transpose(
            1, 2).reshape(B * M, -1, 3)  # B M 1024
